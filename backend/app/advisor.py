...
from .schemas import AdvisorMessage, AdvisorRequest, AdvisorResponse, BeautyID, Product, RecommendationProduct, RecommendationsRequest
...
ADVISOR_JSON_SCHEMA: dict[str, object] = {
    ...
    "schema": {
        ...
        "required": ["message", "quick_actions", "recommended_skus", "routine_steps", "why_this_works", "safety_note"],
    },
}
...
class LLMAdvisorPayload(BaseModel):
    ...
    @validator("quick_actions", "recommended_skus", "routine_steps", pre=True)
    @classmethod
...
def _medical_refusal(provider: str) -> AdvisorResponse:
    ...
    return AdvisorResponse(
        ...
        quick_actions=["мягкая routine", "без отдушек", "SPF", "минимум шагов"],
        recommendations=[],
        recommended_skus=[],
        routine_steps=[],
    ...
    async def respond(self, request: AdvisorRequest) -> AdvisorResponse:
        ...
        if hero:
            answer = (
                ...
                "Это косметический match по Beauty ID, не диагностика. Могу сделать подборку дешевле, мягче по отдушкам или более glow."
            )
...
class OpenRouterAdvisorProvider(AdvisorProvider):
    ...
    def _parse_llm_json(self, content: str) -> LLMAdvisorPayload:
        ...
        try:
            return LLMAdvisorPayload.parse_obj(data)
